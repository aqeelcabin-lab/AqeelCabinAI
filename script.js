// Aqeel Assistant v6 — robust frontend
// Keep this URL ending in /exec only.
...
const ui = {
  es: {
    ...
    faqNotice: "Buscaremos primero en nuestras preguntas frecuentes. Si no hay una respuesta, el asistente de IA podrá ayudar.",
  ...
  en: {
    ...
    faqTitle: "🤖 Ask here",
    faqNotice: "We search the FAQ first. If no answer is found, the AI assistant can help.",
...
function checkAvailability() {
  ...
  for (let i = 0; i < nights; i += 1) {
    if (isDateBooked(addDaysYMD(arrival, i))) {
      unavailable = true;
    }
  }
...
function updateWhatsAppLink(arrival, nights, guests, unavailable) {
  const propertyName = currentProperty === "dome"
    ? "Aqeel Dome"
    : "Aqeel Cabin";
...
function changeProperty() {
  currentProperty = byId("calendarProperty").value === "dome"
    ? "dome"
    : "cabin";
